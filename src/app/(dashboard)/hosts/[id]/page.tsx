    ...
    <div className="space-y-4">
      ...
      <Dialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <DialogContent>
          ...
          {/* F4: warning se il host è linkato a un network_device (FK host_id).
              ON DELETE SET NULL → il device sopravvive ma perde l'ancora. */}
          {host.network_device && (
            <div className="rounded-md border border-amber-300 bg-amber-50 dark:bg-amber-950/30 px-3 py-2 text-sm">
              <p className="font-medium text-amber-900 dark:text-amber-200">⚠️ Questo host è collegato a un dispositivo gestito</p>
              <p className="text-amber-800 dark:text-amber-300 mt-1">
                Il device <span className="font-mono">&quot;{host.network_device.name}&quot;</span> resterà attivo
                ma perderà il riferimento a questo host. Per eliminare anche il device, fallo prima da{" "}
                <Link href={`/devices/${host.network_device.id}`} className="underline">/devices/{host.network_device.id}</Link>.
              </p>
            </div>
          )}
          {host.physical_device_id && (
            <div className="rounded-md border border-blue-300 bg-blue-50 dark:bg-blue-950/30 px-3 py-2 text-sm">
              <p className="text-blue-900 dark:text-blue-200">
                Questo IP è parte di un cluster fisico (#{host.physical_device_id}). Verrà scollegato automaticamente.
              </p>
            </div>
          )}
          <DialogFooter>
