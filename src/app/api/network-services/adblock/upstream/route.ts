...
const DnsRE = /^[a-fA-F0-9:.]+(@\d{1,5})?$/;
const UpstreamSchema = z.object({
  upstream_dns: z.array(z.string().regex(DnsRE)).min(1).max(8),
  bootstrap_dns: z.array(z.string().regex(DnsRE)).max(8).optional(),
});
...
export async function GET() {
  return withTenantFromSession(async () => {
    ...
    try {
      ...
      return NextResponse.json({ ok: true, ...(await client.adblockGetUpstream()) });
    } catch (e) {
  ...
  return withTenantFromSession(async () => {
    ...
    try {
      ...
      return NextResponse.json(
        await client.adblockSetUpstream(parsed.data.upstream_dns, parsed.data.bootstrap_dns),
      );
    } catch (e) {
      if (e instanceof BridgeUnavailableError) {
